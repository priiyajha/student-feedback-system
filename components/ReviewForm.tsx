...
const ReviewForm = ({ subjectId, userId }: ReviewFormProps) => {
    ...
    const mutation = useMutation({
        mutationFn: async (comment: string) => {
            ...
            // This calls your defined API route: /api/feedback/submit-review
    ...
    const isButtonDisabled = mutation.isPending || review.trim().length < 5;

    return (
        <div className="flex flex-col gap-2 mt-2 w-full">
            <textarea
                placeholder="Type your review here..."
                className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-h-[80px] transition"
                rows={3}
                value={review}
